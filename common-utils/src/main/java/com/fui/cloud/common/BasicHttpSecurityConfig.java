...
public class BasicHttpSecurityConfig {
    ...
    public static void basicHttpSecurity(HttpSecurity httpSecurity, JwtAuthenticationConfig config, AuthenticationManager authenticationManager, String type) throws Exception {
        ExpressionUrlAuthorizationConfigurer<HttpSecurity>.ExpressionInterceptUrlRegistry httpSecurityBuilder = httpSecurity.csrf().disable()// 由于使用的是JWT，我们这里不需要csrf
                .logout()
                .disable()
                .formLogin().loginPage(config.getLoginUrl()).defaultSuccessUrl(config.getIndexUrl(), true)
                .permitAll()
                .and()
        ...
        if ("auth".equals(type)) {
            ...
            httpSecurityBuilder
                    .and()
                    .addFilterAfter(new JwtUsernamePasswordAuthenticationFilter(config, authenticationManager), UsernamePasswordAuthenticationFilter.class)
                    .authorizeRequests()
                    .anyRequest()
                    .authenticated();
            ...
            httpSecurityBuilder
                    ...
                    .authorizeRequests()
                    .antMatchers(config.getLoginUrl(), config.getIndexUrl(), config.getGuestUrl())
                    .permitAll();
        }
        // 禁用缓存
